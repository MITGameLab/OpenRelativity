...
3. We have largely re-written the lighting system, assuming that baked lightmaps remain static and stationary in Unity "world frame" coordinates. A notable caveat is that **real-time** reflections are not relativistically accurate, though **baked** reflections are. We notice that full inverse square attenuation (typically corresponding to an attenuation parameter value of "1") is capable of simulating the bending of light by gravity in **real-time** lighting. **Baked** lighting in Unity historically didn't respect inverse square attentuation, but recent versions of Unity expose an experimental option for it, so we have forced this option on and propagated Unity editor-view relativistic shader parameters to shared materials of all renders on GameObject instances with a RelativisticObject component. **Basically, by the current LTS version of Unity and the current version of this repository, you should be able to bake Minkowski or Rindler lightmaps at the click of the usual button.** This won't work with arbitrary ConformalMap instances, but Rindler coordinates are a reasonable approximation to planetary surface gravity over a small region. (It might be possible to bake better light maps, or light maps for other metrics, with *many* separate GameObject/RelativisticObject instances per surface, but each RelativisticObject takes only *one* numerical sample of the background metric, at the center of its Transform or Rigidbody.)
