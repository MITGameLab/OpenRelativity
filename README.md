...
3. We have largely re-written the lighting system, assuming that baked lightmaps remain static and stationary in Unity "world frame" coordinates. A notable caveat is that **real-time** reflections are not relativistically accurate, though **baked** reflections should be self-consistent. We notice that full inverse square attenuation is necessary to simulate the bending of light by gravity, hence **real-time** lighting attenuation should take a parameter value of 1, and **baked-lighting** has inverse square attentuation forced on in code, as it also propagates editor-view player-perspective and RelativisticObject parameters to the lightmapping pipeline. **Basically, just pack your scene densely enough with small RelativisticObject instances, to achieve maximum accuracy, and then hit the button to bake light maps as usual.** It might be possible to bake better light maps, or light maps for other metrics, with *many* separate GameObject/RelativisticObject instances per surface, but each RelativisticObject takes only *one* numerical sample of the background metric, at the center of its Transform or Rigidbody.
