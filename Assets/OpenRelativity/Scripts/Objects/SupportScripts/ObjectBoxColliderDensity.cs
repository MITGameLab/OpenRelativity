    ...
    public class ObjectBoxColliderDensity : MonoBehaviour
    {
                ...
                if (foundCollider)
                {
                    if (isStatic)
                    {
                        toUpdateBox.center = transform.InverseTransformPoint(
                            gameState.conformalMap.WorldToOptical(transform.TransformPoint(origPositions[i]), Vector3.zero, gameState.playerTransform.position, gameState.PlayerVelocityVector)
                       );
                    }
                    else
                    {
                        toUpdateBox.center = transform.InverseTransformPoint(
                            gameState.conformalMap.WorldToOptical(transform.TransformPoint(origPositions[i]), myRO.viw, gameState.playerTransform.position, gameState.PlayerVelocityVector)
                       );
                    }
                }
            }
        ...
        private IEnumerator CPUUpdatePositions()
        {
            ...
            for (int i = 0; i < totalBoxCount; i++)
            {
                ...
                Vector3 newPos = changeTransform.InverseTransformPoint(gameState.conformalMap.WorldToOptical(changeTransform.TransformPoint(origPositions[i]), viw, playerPos, vpw));
                ...
                if (!float.IsInfinity(nanInfTest) && !float.IsNaN(nanInfTest))
                {
                    ...
                    change[i].center = changeTransform.InverseTransformPoint(gameState.conformalMap.WorldToOptical(changeTransform.TransformPoint(origPositions[i]), viw, playerPos, vpw));
                }
            }
