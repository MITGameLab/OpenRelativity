    ...
    public class StaticVoxelTransformer : MonoBehaviour
    {
        ...
        private IEnumerator CPUUpdatePositions()
        {
            ...
            for (int i = 0; i < queuedColliders.Count; i++)
            {
                ...
                Vector3 newPos = changeTransform.InverseTransformPoint(gameState.conformalMap.WorldToOptical(queuedOrigPositions[i], Vector3.zero, playerPos, vpw));
        ...
        private void Cull()
        {
            ...
            if (sphericalCulling)
            {
                ...
                {
                    distSqr = (gameState.conformalMap.WorldToOptical(origPositionsList[i], Vector3.zero, playerPos, playerVel) - playerPos).sqrMagnitude;
