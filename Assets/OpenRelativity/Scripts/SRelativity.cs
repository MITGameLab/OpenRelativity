    ...
    public static class SRelativityUtil
    {
        ...
        public static Vector3 PseudoOpticalToWorld(this Vector3 piw, Vector3 velocity, Vector3 origin, Vector3 playerVel)
        {
            ...
            if (speedr != 0) // If speed is zero, rotation fails
            {
                ...
                //Reverse the Lorentz transform.
                riw.z *= Mathf.Sqrt(1 - (speed * speed));

                //Estimate the time by performing the rest of the forward WorldToOptical map with opposite velocities:
                float c = -(riw.sqrMagnitude); //first get position squared (position dotted with position)
                ...
                riw += tisw * rotateViw;
                riw.z += (((float)speed * spdOfLight) * tisw);
